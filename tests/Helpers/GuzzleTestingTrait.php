...
namespace Trucker\Tests\Helpers;
...
trait GuzzleTestingTrait
{
     ...
     * @param int    $historyLimit number of requests to keep
     */
     ...
     *
     * @param Client $client      Guzzle Client to mock response for
     * @param int    $http_status HTTP status to return
     * @param array  $headers     HTTP Headers for the response
     * @param string $body        HTTP response body
     */
    public function mockHttpResponse($client, $http_status = 200, array $headers = [], $body = '')
    {
        $mock = new MockPlugin();
        $mock->addResponse(
            new Response(
     ...
     * @return Client
     */
    protected function initGuzzleRequestTest(array $config = [])
    {
    ...
    protected function makeGuzzleAssertions(
        ...
        if (!in_array($method, ['GET', 'HEAD', 'TRACE', 'OPTIONS'], true)) {
